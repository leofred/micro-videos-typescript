...
describe('ValidatorRules Unit Tests', () => {
  ...
  test('required validation rule', () => {
    ...
    const error = new ValidationError(`The field property is required`)
  ...
  test('string validation rule', () => {
    ...
    const error = new ValidationError(`The field property must be a string`)
    arrange.forEach((item) => {
      assertIsInvalid({
        value: item.value,
        property: item.property,
  ...
  test('maxLength validation rule', () => {
    ...
    const error = new ValidationError(
      `The field property must be less than 5 characters`
    )
    arrange.forEach((item) => {
      assertIsInvalid({
        value: item.value,
  ...
  test('boolean validation rule', () => {
    ...
    const error = new ValidationError(`The field property must be a boolean`)
    arrange.forEach((item) => {
      assertIsInvalid({
        value: item.value,
        property: item.property,
  ...
  it('should throw a validation error when combine two or more validation rules', () => {
    ...
    }).toThrow(new ValidationError('The field property is required'))

    validator = ValidatorRules.values(5, 'field')
    expect(() => {
      validator.required().string().maxLength(5)
    }).toThrow(new ValidationError('The field property must be a string'))
    ...
    expect(() => {
      validator.required().boolean()
    }).toThrow(new ValidationError('The field property must be a boolean'))
  })
